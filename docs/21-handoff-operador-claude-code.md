...
**Última actualización: 2026-09-07 · Producción: serie 5.4 completa (docs/75) — karaoke v2 por subcadena (blanco/contorno negro solo lo dicho), título IA por claim (rueda deep, «Abascal: ...»), /wiki/video/<slug>/ con dos enlaces por claim (?t= al segundo anterior), y el VIGILANTE DE CHINA (rueda china_guard Haiku; models_for_post desvía a Anthropic todo vídeo que toque a China; prudencia=True si falla). CAZA: la capa SETTING_DEFAULTS pisaba los defaults Qwen — tres capas alineadas. PENDIENTE de David: autorrenovación Token Plan (vence 06-10) · su pregunta sobre Qwen-VL respondida en el chat (propuesta serie 5.5 «la vista»). ⏰ Google: 2/10 (serie 5.4 = 2) (este documento se actualiza en cada parche)**
