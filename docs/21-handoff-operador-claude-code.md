...
**Última actualización: 2026-09-07 · Producción: serie 5.5 completa (docs/76) — relojes por palabra de AssemblyAI (analysis/0020, `word_times`+`data-wt`), karaoke v3 EN FLUJO (intervención negra/blanca restaurada, cada palabra dicha = chip blanco/negro `.kw.dicho`; v1 clip-path y v2 capas ABANDONADOS), banner «Faltan X €» sigue a budget_base_eur, LA VISTA (vision.py: fotograma del segundo con qwen3-vl-plus si la frase apela a lo visible, ajuste `vision_pass`, VL con web=False), y 5.5-E: la wiki gira sobre los INTERLOCUTORES (personas al frente de la portada; en la ficha cada claim con DOS enlaces: /wiki/claim/ y post?t=segundo). LECCIÓN: los reportes de David («karaoke sigue mal», «te has cargado la wiki») eran contra producción con el 5.4 — comprobar QUÉ versión ve el usuario antes de recodificar. PENDIENTE de David: autorrenovación Token Plan (vence 06-10) · mirar el karaoke v3 y la wiki con sus ojos. ⏰ Google: 3/10 (serie 5.5 = 3) (este documento se actualiza en cada parche)**
