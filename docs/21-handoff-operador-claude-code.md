...
- 🧭 **SERIE 5.23 (2026-09-08/09, `cb66472`, docs/88 + §85)**: ver la cabecera de este documento. Estado
  ...
  `{{ RUNPOD_SECRET_HF_TOKEN }}` (probado en CUDA con community-1). PENDIENTE de David: PAT
  `write:packages` para reconstruir/pushear `istt-diarize` con el Dockerfile de secreto (la 4.4-J-slim
  sigue sirviendo con el token viejo YA MUERTO), borrar las etiquetas gordas de ghcr, Google 10/10,
