...
3. Algoritmo dice FACTUAL → post en foro principal en estado **"pendiente de validación"** (transcripción sincronizada y señales ya visibles). **5 usuarios distintos Contribuidor+ deben marcarlo "es factual" en 3 días** → al 5º voto se lanza la **fase cara** (Sonnet + búsquedas adaptativas + wiki + reincidencia), con prioridad si hay señales de manipulación. Sin 5 votos en 3 días → Off-Topic conservando señales. — **ENMENDADO 2026-09-11 (David, literal: «Vamos a cambiar que el trabajo 2 sólo entra si hay X votos de los usuarios para analizarlo profundamente, y están identificados el 66% de los hablantes» + «Quita la ventana de validación»): TODOS los vídeos (opinión incluida) van a «pendiente» al terminar la fase barata; la fase cara arranca ÚNICAMENTE con X votos «Pedir el análisis con fuentes» (`votes_to_validate`, 5 de fábrica; modo arranque: 1 voto de moderador) Y ≥ 66 % de hablantes identificados (`min_identified_speakers_percent`). SIN plazo: la validación no caduca. El piloto automático del 4.4-B y su tope diario quedan RETIRADOS. La etiqueta opinión/hecho del vídeo entero es solo un aviso a moderación; el clasificador de segunda opinión se retira y su rueda pasa al bibliotecario, que comprueba en cada vídeo si el subforo elegido cuadra y lo sugiere a moderación (serie 5.27).**
...
**Otras del quiz de foro/wiki**: subforos 1A (Principal+Off-Topic; 12 temas = etiquetas) — **ENMENDADO 2026-09-08 (David, literal: «Ahora los 12 temas son subforos de Principal, y se pueden seguir añadiendo más, tanto desde el panel de superusuario como aprobando un post de un usuario con una etiqueta diferente a esas 12»): categorías en ÁRBOL (`Category.parent`), categoría obligatoria al publicar, propuesta nueva = post pendiente de aprobación por moderación en /pendiente/ (serie 5.23)** · comentarios Markdown basico 2B · sin hilos sin video 3A · sin privados 4A · avatares 5B con chequeo de vision Haiku (check_avatar; mod retira; AuditLog) · busqueda foro 8A · slugs 9B · interenlazado automatico 10A (_autolink) · cambios recientes 11A · volcado CC-BY-SA solo bajo peticion 12C · tarjeta compartible imagen en 2B (13A) · voto positivo visible, sin negativos — **ENMENDADO 2026-09-08 (David, literal: «los votos positivos y negativos se ven en número, y crea en el panel un setting para como superusuario decidir cuántos negativos provocan que el mensaje se difumine»): ▲/▼ en posts y comentarios con karma ±1 al autor; difuminado desde −5 y plegado desde −10, ajustables en el panel (`karma_fade_threshold`, `karma_fold_threshold`). Trending y «más votados» siguen contando SOLO los ▲ — **ENMENDADO 2026-09-11 (David, literal: «Los votos negativos y positivos no se reinician, y también sirven para "Los más votados" y "Trending" (esto va por votos/hora, implementable ya y ajustable en el panel de superusuario)»): «más votados» = ▲ − ▼; Trending = media de votos por hora (▲ y ▼) en la ventana del panel (`trending_votes_per_hour`, `trending_window_hours`); el contador ▲/▼ vive en la esquina superior izquierda de la caja del post (serie 5.30)**; el reescaneo del post entero — **corregido 2026-09-11 (David: «tiene que ser con votos abajo»)** — lo piden los ▼ (serie 5.29-D)**.
...
- **Portada por secciones**: Recientes / Mas votados (ventana 7 dias, cuenta solo los ▲; desde 5.23 existe tambien el ▼ con karma) /
